...
rDownloadURL = {"main": "https://balkantv.info/toxic/main_xtreamcodes_reborn.tar.gz?dl=0", "sub": "https://balkantv.info/sub/sub_xtreamcodes_reborn.tar.gz"}
...
def prepare(rType="MAIN"):
    ...
    os.system("wget -q -O /tmp/libpng12.deb https://balkantv.info/libb/libpng12-0_1.2.54-1ubuntu1_amd64.deb?dl=1")
...
def configure():
    ...
    os.system("sudo wget -q https://balkantv.info/youtt/youtube-dl -O /usr/local/bin/youtube-dl")
    os.system("sudo chmod a+rx /usr/local/bin/youtube-dl")

def start(first=True):
    if first: printc("Starting BALKAN TV Panel v.4")
...
if __name__ == "__main__":
    printc("BALKAN Panel V4.0", col.OKGREEN, 2)
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if raw_input("  ").upper() == "Y":
                ...
                start()
                printc("BALAKAN TV panel V4.0 completed!", col.OKGREEN, 2)
                printc("Your Admin BALKAN Panel link is:")
